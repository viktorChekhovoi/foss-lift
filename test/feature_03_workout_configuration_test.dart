...
void main() {
  ...
  Finder stepper(Key field, IconData icon) =>
      find.descendant(of: find.byKey(field), matching: find.byIcon(icon));

  group('GZCL progression is configured on the slot', () {
    test('a custom T1 ladder survives saving and reopening', () async {
      final draft =
          ItemDraft.forExercise(await exerciseNamed(db, 'Bench Press'))
            ..setGzclTier(GzclTier.t1)
            ..gzclStages = const [
      ...
      final reopened = ItemDraft.fromView(saved);
      expect(reopened.gzclTier, GzclTier.t1);
      expect(reopened.gzclStages, draft.gzclStages);
      expect(reopened.gzclStage, 0);
    });

    test(
      'a custom T3 final-AMRAP target survives saving and reopening',
      () async {
        final draft =
            ItemDraft.forExercise(await exerciseNamed(db, 'Bench Press'))
              ..setGzclTier(GzclTier.t3)
              ..gzclAmrapTarget = 20;
        ...
        final saved = await itemNamed(db, push, 'Bench Press');
        final reopened = ItemDraft.fromView(saved);
        expect(reopened.gzclTier, GzclTier.t3);
  ...
  group('switching the axis keeps the rates set on the axis left behind', () {
    ...
    test('the set-aside rates outlive the sheet and the app', () async {
      ...
      final reopened = ItemDraft.fromView(
        await itemNamed(db, push, 'Bench Press'),
      );
    ...
    test('the advanced axis keeps its own pair across a save', () async {
      ...
      expect(
        reopened.increment,
        1.25,
        reason: 'the plain weight rates, in use',
      );
    ...
    test(
      'weight → advanced → weight finds the plain rates untouched',
      () async {
        final bench = await exerciseNamed(db, 'Bench Press');
        final draft = ItemDraft.forExercise(bench)
          ..repsMax = 8
        ...
        expect(
          draft.increment,
          1.25,
          reason: 'the plain weight rates, as left',
        );
        expect(draft.deload, 7.5);
    ...
    test(
      'reps → advanced puts the slot on weight and keeps the rep rates',
      () async {
        final bench = await exerciseNamed(db, 'Bench Press');
        final draft = ItemDraft.forExercise(bench)
          ..repsMax = 8
    ...
    test(
      'only the axis in use reaches the database, rep rates included',
      () async {
        final push = await workoutIdNamed(db, 'Push');
        final bench = await exerciseNamed(db, 'Bench Press');
        final draft = ItemDraft.forExercise(bench)
  ...
  group('a rep range keeps its width as progression moves it', () {
    ...
    test(
      'a slot on the advanced axis does not move the range at all',
      () async {
  ...
  group('the sets of a slot can back off, ramp up, or be written out', () {
    ...
    test('back-off opens at the top and takes the percentage off each set', () {
      ...
      expect(
        targets(SetScheme.backOff).map((t) => t.reps),
        [8, 8, 8],
    ...
    test('ramp is the same ladder climbed the other way', () {
      expect(
        targets(SetScheme.ramp).map((t) => t.weightKg),
        [80, 90, 100],
    ...
    test(
      'a custom scheme short of rows falls back to the slot\'s own target',
      () {
        ...
        final got = targets(
          SetScheme.custom,
          sets: 3,
          custom: [const CustomSet(reps: 5, percent: 90)],
        );
    ...
    test('and the floor never loads a deliberately bare bar back up', () {
      ...
      expect(
        targets(SetScheme.backOff, top: 0, floorKg: 20).map((t) => t.weightKg),
        [0, 0, 0],
      );
    });
    ...
    test('a slot keeps its scheme through a save and a reload', () async {
      ...
      await db.replaceWorkoutItems(
        wid,
        itemCompanions([draft], workoutId: wid),
      );
      ...
      expect(
        ItemDraft.fromView(await itemNamed(db, wid, 'Bench Press')).scheme,
        SetScheme.backOff,
      );
    });
  });
  ...
  group('the scheme is what the board hydrates from', () {
    ...
    test(
      'a back-off set is judged against its own weight, not the top one',
      () async {
        final container = containerFor(db);
        addTearDown(container.dispose);
  ...
  group('the Target card opens on sets, reps and rest', () {
    Future<void> openBench(
      WidgetTester tester,
      ProviderContainer container, {
      ItemDraft Function(ItemDraft)? configure,
    }) async {
      final bench = (await tester.runAsync(
        () async =>
            ItemDraft.forExercise(await exerciseNamed(db, 'Bench Press')),
    ...
    testWidgets('RPE is optional and stays behind Advanced', (tester) async {
      final container = containerFor(db);
      addTearDown(container.dispose);
      await openBench(tester, container);
      expect(
        find.textContaining('EFFORT TARGET', findRichText: true),
        findsNothing,
      );
      ...
      expect(
        find.text('None'),
        findsOneWidget,
        reason: 'ordinary slots do not prescribe an effort rating',
      );
    ...
    testWidgets(
      'picking back-off offers a percentage and reads the ladder back',
      (tester) async {
        final container = containerFor(db);
        addTearDown(container.dispose);
        final l10n = l10nFor();
        ...
        expect(
          find.text(l10n.itemEditorSchemeCustom),
          findsOneWidget,
    ...
    testWidgets('a timed slot has no advanced half at all', (tester) async {
      final container = containerFor(db);
      addTearDown(container.dispose);
      final plank = (await tester.runAsync(
        () async => ItemDraft.forExercise(await exerciseNamed(db, 'Plank')),
      ))!;
      await openSheet(tester, container, [plank]);

      expect(fieldLabel(l10nFor().itemEditorHold), findsOneWidget);
      expect(
        find.byKey(kAdvancedToggleKey),
        findsNothing,
  ...
  group(
    ...
    () {
      ...
      testWidgets(
        'a slot on the advanced axis opens Advanced already expanded',
        (tester) async {
          final container = containerFor(db);
          addTearDown(container.dispose);
          await openDraft(
      ...
      testWidgets('the Target card no longer offers it', (tester) async {
        ...
        expect(
          fieldLabel(l10nFor().itemEditorRepRange),
          findsOneWidget,
      ...
      testWidgets('the tick sits under the line saying what it does', (
        ...
        expect(
          find.text(l10n.itemEditorAddWeightAtTopHint),
          findsOneWidget,
          reason: 'what the combination does, whether or not it can be ticked',
        );
        expect(
          find.text(l10n.itemEditorRangeClimbNeedsRange),
          findsNothing,
      ...
      testWidgets(
        'a fixed rep count gets no toggle, and a greyed pill instead',
        (tester) async {
          final container = containerFor(db);
          addTearDown(container.dispose);
          ...
          expect(
            ...
            findsOneWidget,
            reason:
                'a control that vanishes entirely teaches nobody where it '
                'went — the pill on the axis row is what stays',
          );
          expect(
            tester.widget<EditorPill>(find.byKey(kModeAdvancedKey)).onTap,
            isNull,
      ...
      testWidgets('a slot on the reps axis with a range may take it', (
        ...
        expect(
          find.text(l10nFor().itemEditorRangeClimbNeedsRange),
          findsNothing,
        );
      ...
      test(
        'the tick, the rates and the goal round-trip through a save',
        () async {
          final push = await workoutIdNamed(db, 'Push');
          final bench = await exerciseNamed(db, 'Bench Press');
      ...
      test(
        'an untouched slot saves with it off, at the rep defaults',
        () async {
          final push = await workoutIdNamed(db, 'Push');
          final bench = (await itemNamed(db, push, 'Bench Press')).item;
          expect(bench.repsMax, 8);
  ...
  group(
    'ticking to failure greys the rep range rather than taking it away',
    () {
      ...
      testWidgets('the stepper goes dead and keeps the bound it had', (
        ...
        expect(
          find.text('8'),
          findsWidgets,
          reason: 'the bound is still on screen',
        );

        await tester.tap(
          stepper(kRepRangeFieldKey, Icons.add),
          warnIfMissed: false,
        );
        await tester.pumpAndSettle();
        await tester.tap(
          stepper(kRepRangeFieldKey, Icons.remove),
          warnIfMissed: false,
        );
        await tester.pumpAndSettle();
  ...
  group('a bar-loaded slot cannot be authored under its bar', () {
    test('a weight below the bar is held at the bar on the way in', () async {
      ...
      expect(
        (await itemNamed(db, push, 'Bench Press')).item.suggestedWeight,
        20,
    ...
    test('a machine slot has no floor, and a held one has no weight', () async {
      ...
      expect(
        (await itemNamed(db, push, 'Triceps Pushdown')).item.suggestedWeight,
        0,
        reason: 'a stack can read zero',
      );
      expect(
        (await itemNamed(db, push, 'Plank')).item.suggestedWeight,
        isNull,
  ...
  group('adding an exercise opens its config', () {
    ...
    testWidgets('picking a movement lands on its sheet, not back on the list', (
      ...
      expect(
        find.byType(ExercisePicker),
        findsNothing,
        reason: 'the picker closes behind the sheet',
      );
      expect(
        find.byKey(kStepUpFieldKey),
        findsOneWidget,
        reason:
            'no configuration sheet opened: adding an exercise is two '
  ...
  group('a slot can be joined to the one above it as a superset', () {
    ...
    testWidgets('the first slot of a workout has nothing above it to join to', (
      ...
      final container = containerFor(db);
      addTearDown(container.dispose);
      final drafts = await draftsFor(tester, ['Bench Press', 'Overhead Press']);

      await openSheet(tester, container, drafts);
      expect(
        find.byKey(_supersetCheck),
        findsNothing,
    ...
    testWidgets('a superset reads as a group in the exercise list', (
      ...
      await pumpList(
        tester,
        container,
        await draftsFor(tester, ['Bench Press', 'Overhead Press'], joined: {1}),
      );
    ...
    testWidgets('a joined slot dragged to the top of the list is unjoined', (
      ...
      expect(drafts.map((d) => d.name), [
        'Overhead Press',
        'Bench Press',
      ], reason: 'the dragged row did not move');
      expect(
        drafts.map((d) => d.supersetWithPrevious),
        [false, false],
