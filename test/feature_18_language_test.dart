...
const Map<String, Set<String>> _keptInEnglish = {
  'uk': {
    'commonAppName',
    'exerciseFormDemoLinkHint',
    'seedRoutine531BoringButBig',
    'seedRoutine531FirstSetLast',
    'seedRoutineDumbbellStopgap',
    'seedRoutinePpl',
    'seedRoutineSheiko2932',
    'seedRoutineStartingStrength',
    'seedRoutineStrongLifts5x5',
    'themeChannelBlue',
    'themeChannelGreen',
    'themeChannelRed',
    'videoSettingsHeight',
  ...
  'es': {
    ...
    'exerciseDetailDemo',
    'exerciseFormDemoLinkHint',
    'exerciseFormMeasureReps',
    'itemEditorAmountReps',
    'itemEditorModeReps',
    'itemEditorReps',
    'itemEditorSecondsSuffix',
    'itemEditorSuffixReps',
    'muscleCardio',
    'muscleCore',
    'seedRoutine531BoringButBig',
    'seedRoutine531FirstSetLast',
    ...
    'themeChannelGreen',
    'themeChannelRed',
    'themePreviewRepsHeader',
    'todayStatReps',
    'unitSecondsShort',
    'videoSettingsHeight',
    'videoSettingsMinutes',
...
bool _isFormatOnly(String message) => !RegExp(
  r'[A-Za-z]',
).hasMatch(_arguments(message).replaceAll(RegExp(r'\{[^{}]*\}'), ''));
...
Future<void> _switchTo(
  ...
  await pumpUntil(
    tester,
    () => container.read(activeLocaleProvider) == localeFromTag(tag),
  );
  await tester.pump();
}
...
void main() {
  ...
  group('which language the app renders in', () {
    ...
    test('every language names itself, and only itself', () {
      for (final locale in kSupportedLocales) {
        expect(
          kLanguageNames[localeTag(locale)],
          isNotNull,
      ...
      expect(
        kLanguageNames,
        hasLength(kSupportedLocales.length),
    ...
    test('a stored choice wins over the phone', () {
      ...
      expect(
        resolveLocale('en', const [Locale('uk')]),
        const Locale('en'),
        reason: 'choosing English on a Ukrainian phone has to stick',
      );
      expect(
        resolveLocale('pt_BR', const [Locale('pt')]),
        const Locale('pt', 'BR'),
      );
    });

    test(
      'a stored tag for a language we no longer ship is as good as unset',
      () {
        // The shape a removed language leaves behind in Settings.
        expect(
          resolveLocale('fr', const [Locale('uk')]),
          const Locale('uk'),
    ...
    test('a Brazilian phone gets Brazilian, and everyone else European', () {
      expect(
        resolveLocale(null, const [Locale('pt', 'BR')]),
        const Locale('pt', 'BR'),
      );
      expect(
        resolveLocale(null, const [Locale('pt', 'PT')]),
        const Locale('pt'),
      );
      expect(resolveLocale(null, const [Locale('pt')]), const Locale('pt'));
      expect(
        resolveLocale(null, const [Locale('pt', 'AO')]),
        const Locale('pt'),
    ...
    test(
      'a phone we cannot answer, or one that says nothing, gets English',
      () {
        expect(resolveLocale(null, const [Locale('fr')]), const Locale('en'));
        expect(
          resolveLocale(null, const [Locale('de'), Locale('fr')]),
          const Locale('en'),
        );
    ...
    test('the phone list is walked in preference order', () {
      ...
      expect(
        resolveLocale(null, const [Locale('de'), Locale('es')]),
        const Locale('es'),
      );
    });
    ...
    test(
      'a language already chosen is not overwritten on the next launch',
      () async {
  ...
  group('the five catalogues agree with each other', () {
    ...
    for (final locale in kSupportedLocales.where(
      (l) => l != const Locale('en'),
    )) {
      ...
      test('$tag has a catalogue on disk', () {
        expect(
          ...
          isNotNull,
          reason:
              'lib/l10n/app_$tag.arb is missing, so ${kLanguageNames[tag]} '
      ...
      test('$tag interpolates the same placeholders, key for key', () {
        ...
        expect(
          ...
          isEmpty,
          reason:
              'a placeholder dropped in translation renders as literal '
              'braces: ${wrong.join("; ")}',
        );
      });
      ...
      test('$tag translates rather than copying the English', () {
        ...
        expect(
          ...
          isEmpty,
          reason:
              '$tag left these as the English string: '
              '${untouched.join(", ")}',
        );
      });
    }
    ...
    test('a starter movement has a key, and a key has a string', () {
      ...
      final unbacked =
          keys
              .where(
                (k) => seededName(english, k, '\u0000none') == '\u0000none',
              )
              .toList()
            ..sort();
      expect(
        ...
        isEmpty,
        reason:
            'these seed keys have no string behind them: '
            '${unbacked.join(", ")}',
      );
    });
  });
  ...
  group('the picker', () {
    ...
    testWidgets('it offers the five languages and nothing else', (
      tester,
    ) async {
      ...
      for (final locale in kSupportedLocales) {
        expect(
          find.text(kLanguageNames[localeTag(locale)]!),
          findsOneWidget,
    ...
    testWidgets('and the choice survives leaving the screen', (tester) async {
      ...
      await pumpUntil(
        tester,
        () => container!.read(localeTagProvider).value == 'pt_BR',
      );
  ...
  group('changing the language repaints at once', () {
    testWidgets('the screen you are on re-reads, with no restart', (
      ...
      expect(
        find.text(uk.languageTitle),
        findsWidgets,
        reason: 'the tree rebuilt against the new locale',
      );
      expect(
        find.text(en.languageTitle),
        findsNothing,
  ...
  group('muscle groups and equipment', () {
    testWidgets('the filter buttons read in the chosen language', (
      ...
      for (final group in kMuscleGroups) {
        expect(
          find.text(muscleGroupLabel(uk, group)),
          findsWidgets,
    ...
    testWidgets('the exercise form offers them in the chosen language', (
      ...
      for (final group in kMuscleGroups) {
        expect(
          find.text(muscleGroupLabel(es, group)),
          findsWidgets,
      ...
      for (final kind in kEquipmentTypes) {
        expect(
          find.text(equipmentLabel(es, kind)),
          findsWidgets,
  ...
  group('the starter library', () {
    testWidgets('a seeded movement reads in the chosen language, everywhere', (
      ...
      expect(
        ...
        findsWidgets,
        reason:
            'the library renders from the seed key, not from the row, '
  ...
  group('what a slot is aiming at', () {
    ...
    Future<int> onlySlot(
      ItemDraft Function(Exercise) draft, {
      ...
      await db.replaceWorkoutItems(
        push,
        itemCompanions([
          draft(await exerciseNamed(db, exercise)),
        ], workoutId: push),
      );
      return push;
    }
    ...
    testWidgets('a to-failure slot says so in the chosen language', (
      ...
      final uk = await _stringsFor(const Locale('uk'));
      await tester.pumpWidget(
        _liveApp(container!, WorkoutDetailScreen(workoutId: push)),
      );
      await tester.pump();
      await _switchTo(tester, db, container!, 'uk');

      expect(
        find.text(uk.targetSetsReps(3, uk.targetFailure)),
        findsOneWidget,
  ...
  group('the demo routines', () {
    ...
    testWidgets(
      'renaming a training day makes the name yours in every language',
      (tester) async {
        tester.view.physicalSize = const Size(400, 1400);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.reset);
    ...
    test(
      'every seeded programme and training day renders from its key',
      () async {
    ...
    test(
      'a programme named after its author reads the same in every language',
      () async {
        ...
        for (final key in ['starting_strength', 'stronglifts_5x5']) {
          ...
          for (final locale in kSupportedLocales) {
            expect(
              seededName(await _stringsFor(locale), key, key),
              english,
        ...
        expect(
          seededName(l10nFor(), 'starting_strength', '?'),
          'Starting Strength',
        );
        expect(
          seededName(l10nFor(), 'stronglifts_5x5', '?'),
          'StrongLifts 5x5',
        );
      },
    );
    ...
    testWidgets('a beginner programme reads in the chosen language', (
      ...
      expect(
        find.text(seededName(uk, 'full_body_3x', 'Full Body 3x')),
        findsWidgets,
        reason: 'the programme names itself in Ukrainian',
      );
  ...
  group('history', () {
    ...
    testWidgets('a session logged in English reads in Ukrainian afterwards', (
      ...
      expect(
        find.text(seededName(uk, 'push', 'Push')),
        findsWidgets,
  ...
  group('a locale that answers nothing', () {
    test('every shipped language loads through the app delegate', () {
      for (final locale in kSupportedLocales) {
        expect(
          ...
          isTrue,
          reason:
              '${localeTag(locale)} is offered in the picker but the '
        ...
        for (final probe in _probes.entries) {
          ...
          if (!theirs.containsKey(probe.key)) {
            expect(
              ...
              probe.value(en),
              reason:
                  '$tag does not answer ${probe.key}, so it must fall '
    ...
    test(
      'a language we do not ship is rendered in English, not refused',
      () async {
  ...
  group('nothing is left hard-coded', () {
    test('no screen or widget holds a user-facing string of its own', () {
      ...
      if (offenders.isNotEmpty) {
        fail(
          '${offenders.length} user-facing strings still live in the widget '
  ...
  group('dates and numbers follow the language', () {
    ...
    testWidgets('history writes its month in the language', (tester) async {
      ...
      expect(
        find.text(DateFormat('MMM', 'uk').format(when).toUpperCase()),
        findsWidgets,
        reason: 'a month name is a translated word like any other',
      );
      expect(
        find.text(DateFormat('MMM', 'en').format(when).toUpperCase()),
        findsNothing,
      );
