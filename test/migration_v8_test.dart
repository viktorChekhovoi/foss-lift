...
/// The v8 schema, verbatim — an install with a bar per exercise and a rack per
/// unit, but no notion of a colour theme. v8→v9 adds the theme choice and a
/// slot for a custom palette. The promise is that the upgrade changes nothing
/// visible: both new columns are nullable and null means "nothing chosen", so
/// the default preset (Ignition) still paints the app.
const _v8Ddl = [
...
/// A v8 install with a couple of preferences already set, so the migration can
