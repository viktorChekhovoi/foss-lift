...
│   ├── percentage_base.dart      Which lift a movement's percentages are of —
│   │                             a front squat off the squat, and the grouping
│   │                             the training-max screen is a list of
...
├── l10n/short/app_*.arb          Labels up to three visible words ← source
├── l10n/long/app_*.arb           Longer UI copy ← source
├── l10n/app_*.arb                GENERATED merged inputs for gen_l10n
...
**A cycle progresses on two clocks.** A slot running one advances a week every
session and moves its weight once, on the session that brings the cycle round:
up by the step if nothing in the cycle came up short, down by the back-off if
the misses reach the slot's threshold, held in between (`stepCycle`). That
weight is a *training max* — every set is a percentage of it — so the
"loading the bar past the suggestion is itself progression" rule is off there:
a heavy week must not be read as the max having moved. See section 22 of the
feature catalogue.
...
the settings row — and **no routines at all**. The thirteen programs the app ships
...
| `/routine/:id/training-maxes` | training_max_screen | One field per lift, written onto every slot in the routine whose percentages come from it |
...
- **`data/set_scheme.dart`** — how a slot's sets differ from one another: flat,
  back-off, ramp, rows written out per set, or a **cycle** — several sets of
  those rows, a week each, taken one per session. A written-out row asks for a
  count, a range, or a minimum with no top (`amrap` — the "5+" set), and
  `CustomSet.goalReps`/`minReps` are how the rest of the app asks which. A
  cycle's set count comes from the week rather than from `targetSets`; every
  reader goes through `WorkoutItemTarget.setCount`. A scheme is stored as *shape and
