...
| `Settings`     | Single-row (id=1) app prefs. `weightUnit`, `activeRoutineId`†, and the layoff rules `layoffDays`/`layoffPercent` |
...
| `/settings` | settings_screen | kg/lb toggle, layoff deload rules |
...
- **A new persisted field/table** → edit `data/database.dart`, rerun
  ...
  `schemaVersion` (currently **6**) and add an `onUpgrade` step. Migrations are
