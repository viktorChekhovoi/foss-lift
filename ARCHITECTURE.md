...
│   ├── warmup.dart               The warm-up ramp: working weight → lighter sets
...
- **Warm-ups ride alongside the working sets, never among them.** A weight-based
  ...
  `start()` from `data/warmup.dart` — ascending percentages of the working
  weight, reps dropping as the load climbs, the empty bar as the floor for a
  barbell lift. They live in `ExerciseEntry.warmups`, a list kept apart from
