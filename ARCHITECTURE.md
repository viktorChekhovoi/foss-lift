...
| `Routines`     | A programme. name, `seedKey` (nullable — which demo programme this is; **cleared on rename**, so a routine you have named stops following the language), colorHex, position, restSeconds (default rest), plus its weekly schedule: `scheduleDays` (day bitmask) and `reminderMinutes` (nullable — no reminder unless asked for) |
...
- **Units** (`util/units.dart`): `toDisplayWeight`, `toKg`, `unitSuffix`. Any
  ...
  are formatted by `util/format.dart`, which goes through `intl` so the decimal
  mark follows the language.
- **Language** (`util/locales.dart`, `l10n/*.arb`): the app ships in English,
  Ukrainian, Spanish and both Portugueses, following the phone unless the user
  picks one. Screens read every string from `AppLocalizations.of(context)`;
