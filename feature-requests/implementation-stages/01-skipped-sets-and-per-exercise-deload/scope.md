...
- `lib/widgets/workout_items_editor.dart` — carry eligibility, period response state and reduction baselines through `ItemDraft` and replacement writes for existing exercise slots, with baseline invalidation on target/axis edits; initialize genuinely new slots separately.
