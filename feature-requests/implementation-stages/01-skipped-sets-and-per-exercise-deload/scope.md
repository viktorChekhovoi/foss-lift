...
20. Section 06's `code:` and `where:` metadata identify `lib/data/database.dart` for offer queries/application and `lib/widgets/start_workout.dart` for the Start dialog; the settings and pure-rule paths remain accurate. Review these references against the actual call path as well as running `--check`, which detects missing files but cannot detect an existing file attributed the wrong responsibility.
...
**Catalogue and copy**
- `features/catalogue/04-live-session.yaml`, `05-progression.yaml`, `06-layoff-deloads.yaml`, `features/concepts.yaml` — include section 06's `code:` / `where:` metadata correction alongside its entry edits
