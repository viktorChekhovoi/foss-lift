...
This requires a schema increment and a new additive `onUpgrade` rung (currently version 18 → 19; use the next version at implementation time). Existing rows start with null response state and reduction baseline; the old build recorded no decisions, so do not fabricate responses or rewrite historical targets. Preserve this state through `ItemDraft` and `replaceWorkoutItems` under the edit rules above, even though that path replaces row ids. New slots and routine-code imports start with null response state and baseline, and neither travels in routine share codes. Full database backups carry both; old backups upgrade with null state. Regenerate `database.g.dart` with build_runner, keep shipped migration rungs and wire formats intact, and update the table map in `ARCHITECTURE.md`.
...
16. Upgrading a shipped database or restoring an old backup preserves history, routines, settings and targets and initializes response state and baseline to null; historical performed sets determine the first offer. A backup from the new build retains the handled period, next boundary and baseline on restore. Existing routine codes remain readable and do not acquire training state.
...
**Data**
- `lib/data/database.dart` — `advanceProgression` (~2813), `_advanceGzcl`, `_advanceCycle`, `layoffFor` (~3077), `applyLayoffDeload` (~3096), `lastTrainedAt` (~3203); the new per-exercise query alongside `lastLoggedWeight` (~3379), transactional period responses and reduction baselines, `WorkoutItems` state and the next additive migration rung.
...
- `lib/widgets/workout_items_editor.dart` — carry period response state and reduction baselines through `ItemDraft` and replacement writes for existing exercise slots, with baseline invalidation on target/axis edits.
- `lib/services/backup_service.dart`, `lib/data/backup_archive.dart` — verify period response state and reduction baselines survive database backup/restore and older schemas upgrade; retain existing archive and routine-code formats.
