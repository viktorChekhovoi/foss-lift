...
1. An in-scope exercise whose planned sets are all skipped comes out of Finish with its `suggestedWeight`, `repsMin`/`repsMax`, `repsTarget`, `successStreak` and `failStreak` byte-for-byte as they went in, and does not appear in the recap as a success or a miss.
...
| Verdict rules — `ExerciseEntry`, `advanceProgression`, the Time/cycle/GZCL/RPE boundary, the two interacting rules | 2–3 |
